...
)
{
    ...
    // 中轴接缝的阶跃尺度可达上百米，小核模糊压不住 → 潮头穿缝跳变。
    // 可分离盒式模糊：H、V 各一遍，大 radius，多次迭代逼近宽高斯，成本远低于 NxN。
    const uint32_t res = config.resolution;
    const int radius = 12;      // 单遍盒式半径(texel)。worldSize/res 米/texel × 12 = 有效核
    const int iterations = 4;   // 迭代次数：越多越接近高斯

    std::vector<glm::vec4> tmp(data.field.size());
    auto boxBlur1D = [&](bool horizontal){
        for(uint32_t y = 0; y < res; ++y){
            for(uint32_t x = 0; x < res; ++x){
                glm::vec4 sum(0.0f);
                float wsum = 0.0f;
                for(int k = -radius; k <= radius; ++k){
                    int sx = horizontal ? static_cast<int>(x) + k : static_cast<int>(x);
                    int sy = horizontal ? static_cast<int>(y) : static_cast<int>(y) + k;
                    if(sx < 0 || sy < 0 ||
                       sx >= static_cast<int>(res) || sy >= static_cast<int>(res)) continue;
                    sum += data.field[static_cast<uint32_t>(sy) * res + static_cast<uint32_t>(sx)];
                    wsum += 1.0f;
                }
                tmp[y * res + x] = sum / wsum;
            }
        }
        data.field.swap(tmp);
    };
