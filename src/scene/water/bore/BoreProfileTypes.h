...
struct BoreWaveProfileConfig
{
    ...
    // ===== 空间范围 =====
    float profileHalfWidth = 240.0f;     // 剖面覆盖的半宽度(米)，从波前峰向前后各延伸30米
                                         // 决定了波前影响的总宽度范围

    // ===== 潮头主体形状 =====
    float crestHeight = 2.2f;            // 波峰最大高度(米)，潮头最高点相对于水平面的垂直位移
    float forwardDisplacement = 0.6f;    // 最大前向水平位移(米)，潮头向前推进时的最大推拉量
    float crestWidth = 32.0f;            // 波峰宽度(米)，控制潮头尖峰的横向范围
    float rearSlopeLength = 220.0f;      // 波峰后半段平滑回落长度(米)，越大越不像一堵墙
    float rearTroughDepth = 0.0f;       // 后坡波谷深度(米)，潮头后方的水面下凹程度
    float hydraulicRiseScale = 0.0f;     // Profile 内部水跃台阶强度；通常为0，宏观水位由 waterRise 负责
