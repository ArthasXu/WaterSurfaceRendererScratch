...
void Stage6CPUFFTApp::CreateTessendorfSource()
{
    ...
    // 频域网格分辨率（N x N），决定波浪细节的丰富程度和 FFT 计算量
    params.resolution = m_FFTResolution;

    // 海面补丁的物理边长（米），决定最大波长和频域采样间距
    params.patchLength = m_PatchLength;

    // 主风向（二维单位向量），控制波浪能量的方向性，这里略微偏向 x 轴
    params.windDirection = glm::normalize(glm::vec2(1.0f, 0.25f));

    // 风速（米/秒），影响 Phillips 谱的强度，风速越大波浪越高
    params.windSpeed = 25.0f;

    // 全局频谱振幅缩放因子，用于整体调节波高
    params.spectrumAmplitude = 2.5f;

    // 短波阻尼系数，抑制高频毛细波的强度，使海面更平滑自然
    params.shortWaveDamping = 0.001f;

    // 重力加速度（m/s^2），用于色散关系 ω = √(g k)
    params.gravity = 9.81f;

    // choppy 水平位移强度系数（典型值 0.5~1.5），越大波峰越尖锐
    params.choppyLambda = 1.0f;

    // 逆风方向波浪能量的额外衰减因子，减少背风面的波浪
    params.oppositeWindDamping = 0.07f;

    // 随机种子，确保同一种子生成完全相同的海面（可重现性）
    params.randomSeed = 1337;

    m_Tessendorf = std::make_unique<water::WSTessendorfCPU>(params);
}
...
void Stage6CPUFFTApp::CreateCPUFFTFrameResources()
{
    ...
    for(uint32_t i = 0; i < GetMaxFramesInFlight(); i++){
        ...
        // 创建位移数据 staging buffer，CPU 可映射写入，GPU 用作传输源
        resources.displacementStaging = std::make_unique<vkp::Buffer>(
            GetPhysicalDevice(),
            GetDevice(),
            fieldSize,
        ...
        // 保持持久映射，后续直接通过指针拷贝 FFT 计算结果
        resources.displacementStaging->Map();

        // 创建法线辅助数据 staging buffer，同样 CPU 可写、GPU 用于传输
        resources.normalAuxStaging = std::make_unique<vkp::Buffer>(
            GetPhysicalDevice(),
            GetDevice(),
            fieldSize,
            VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        ...
        resources.normalAuxStaging->Map();

        // 动态图像的用途：作为传输目标（接收 staging 数据）和着色器采样资源
        VkImageUsageFlags usage =
            VK_IMAGE_USAGE_TRANSFER_DST_BIT |
            VK_IMAGE_USAGE_SAMPLED_BIT;

        // 创建设备本地的动态位移图像
        resources.displacementImage = std::make_unique<water::DynamicImage2D>(
            GetPhysicalDevice(),
            GetDevice(),
            m_FFTResolution,
            m_FFTResolution,
        ...
        // 创建设备本地的动态法线辅助图像
        resources.normalAuxImage = std::make_unique<water::DynamicImage2D>(
            GetPhysicalDevice(),
            GetDevice(),
            m_FFTResolution,
            m_FFTResolution,
...
void Stage6CPUFFTApp::CreateDescriptorSets()
{
    ...
    for(uint32_t i = 0; i < GetMaxFramesInFlight(); i++){
        ...
        VkDescriptorImageInfo displacementInfo =
            m_CPUFFTFrames[i].displacementImage->GetDescriptorInfo(*m_FFTSampler);

        // 4. 准备 法线辅助图 的绑定信息，同样使用 FFTSampler
        VkDescriptorImageInfo normalAuxInfo =
            m_CPUFFTFrames[i].normalAuxImage->GetDescriptorInfo(*m_FFTSampler);
        ...
        bool success = vkp::DescriptorWriter(*m_DescriptorSetLayout, *m_DescriptorPool)
            ...
            .WriteBuffer(1, &waterParamsInfo) // 写入 binding = 1
            .WriteImage(2, &displacementInfo) // 写入 binding = 2
            .WriteImage(3, &normalAuxInfo)    // 写入 binding = 3
...
void Stage6CPUFFTApp::Update(core::Timestep timestep)
{
    ...
    if(m_Tessendorf){
        ...
        const auto& frame = m_Tessendorf->GetFrame();
        ...
        std::cout
            << "time = " << m_Time
            << ", checksum = " << m_Tessendorf->ComputeFrameChecksum()
            << ", height range = ["
            << minHeight
            << ", "
            << maxHeight
...
{
    const water::CPUWaterSurfaceFrame& frame = m_Tessendorf->GetFrame();
    ...
    resources.displacementStaging->CopyToMapped(
        frame.displacement.data(),
        fieldSize
    );

    resources.normalAuxStaging->CopyToMapped(
...
{
    water::WaterParamsUBO ubo{};
    ubo.patchLengths = glm::vec4(m_PatchLength, 0.0f, 0.0f, 0.0f);
    ubo.amplitudeScales = glm::vec4(1.0f, 0.0f, 0.0f, 0.0f);
    ubo.metadata = glm::ivec4(1, 0, 0, 0);
    ubo.simulation = glm::vec4(m_Time, 1.0f, 1.0f, static_cast<float>(m_DebugMode));
...
void Stage6CPUFFTApp::Render(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
    ...
    resources.displacementImage->RecordUpload(
        commandBuffer,
        *resources.displacementStaging
    );

    resources.normalAuxImage->RecordUpload(
        commandBuffer,
        *resources.normalAuxStaging
    );
