...
void Stage8BoreProfileApp::ShutdownApp()
{
...
void Stage8BoreProfileApp::CreatePipelines()
{
    ...
    m_SolidPipeline = std::make_unique<vkp::Pipeline>(
        GetDevice(),
        GetRenderPass(),
        "shaders/water/stage8_bore_profile.vert.spv",
        "shaders/water/stage8_bore_profile.frag.spv",
        config
    );
...
void Stage8BoreProfileApp::CreateDescriptorSetLayout()
{
    // 2 个 UBO + 6 个纹理
...
void Stage8BoreProfileApp::CreateWaterGrid()
{
...
void Stage8BoreProfileApp::CreateSamplers()
{
...
void Stage8BoreProfileApp::CreateDescriptorPool()
{
...
// 描述符集布局：m_DescriptorSetLayout（在 Stage8BoreProfileApp 中创建，与计算管线的布局不同）。
void Stage8BoreProfileApp::CreateDescriptorSets()
{
    // 为每个飞行帧预留一个描述符集的空位
...
void Stage8BoreProfileApp::UpdateCamera(float deltaTime)
{
...
void Stage8BoreProfileApp::Update(core::Timestep timestep)
{
...
void Stage8BoreProfileApp::UpdateWaterParamsUniformBuffer(uint32_t frameIndex)
{
...
void Stage8BoreProfileApp::UpdateBoreFrontUniformBuffer(uint32_t frameIndex)
{
    glm::vec2 direction =
...
void Stage8BoreProfileApp::UpdateWindowTitle()
{
...
void Stage8BoreProfileApp::OnFramebufferResize(int width, int height)
{
    core::Application::OnFramebufferResize(width, height);
}

void Stage8BoreProfileApp::OnKey(int key, int scancode, int action, int mods)
{
...
void Stage8BoreProfileApp::OnMouseMove(double x, double y)
{
...
void Stage8BoreProfileApp::OnMouseButton(int button, int action, int mods)
{
